...
def passed_students(attempts: list, course: str):
    student_passed = list(filter(lambda attempt: attempt.grade > 0, attempts))
    student_passed_course = list(filter(lambda attempt: attempt.course_name == course, student_passed))
    return sorted(list(map(lambda s: s.student_name, student_passed_course)))
